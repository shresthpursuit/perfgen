...
- **Never run the tool against a real NFR spreadsheet** - `data/specs/sample_filled.xlsx`,
  `data/specs/template.xlsx`, or any spreadsheet a user supplies - **except when the user
  explicitly asks for that run.** Not while building, not to check your work, and never as a
  side effect of finishing a milestone. Test with fixtures generated by the test suite instead
...
- **No secret values ever written** to disk, IR, logs, or JMX. Reference names only; the JMX reads
  credentials at run time via `${__groovy(System.getenv('NAME'))}` - note `${__env(...)}` is NOT a
  core JMeter function and is silently passed through as literal text.
