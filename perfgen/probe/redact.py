...
def _redact_by_name(key: str, exempt: set[str] | frozenset[str] | None) -> bool:
    """Redact by name unless the probe filled this field from an observed response."""
    if exempt and _normalise_key(key) in exempt:
        return False
...
class Redactor:
    ...
    def body(
        ...
        correlated_keys: frozenset[str] | set[str] = frozenset(),
    ) -> str | None:
        ...
        `correlated_keys` names fields the probe filled from a value it had just seen in an earlier
        *response*. Those are server-generated by definition, and the name heuristic must not
        destroy them: `loginToken` and Entra's `flowToken` both contain "token", so redacting them
        by name removed the very evidence correlation exists to find, and the scan then reported
        nothing to correlate. The by-value pass is untouched, so a `{secret:...}` value stays
        redacted whatever its field is called - the guarantee still holds, only the heuristic
        yields.
        """
        if body is None:
            return None
        ...
        exempt = {_normalise_key(k) for k in correlated_keys}
        redacted = self._structured_body(self.scrub(body) or "", content_type, exempt)
        return self.scrub(redacted)

    def _structured_body(
        self, body: str, content_type: str | None, exempt: set[str] | None = None
    ) -> str:
        ...
        if "json" in kind or stripped.startswith(("{", "[")):
            try:
                return json.dumps(_redact_json(json.loads(body), self.scrub, exempt or set()))
        ...
        if "form-urlencoded" in kind or looks_form_encoded(body):
            try:
                ...
                if pairs:
                    return urlencode(
                        [
                            (k, REDACTED if _redact_by_name(k, exempt) else (self.scrub(v) or ""))
                            for k, v in pairs
                        ]
                    )
            except ValueError:
...
def _redact_json(node, scrub, exempt=frozenset()):
    ...
    if isinstance(node, dict):
        return {
            key: REDACTED
            if _redact_by_name(str(key), exempt)
            else _redact_json(value, scrub, exempt)
            for key, value in node.items()
        }
    if isinstance(node, list):
        return [_redact_json(item, scrub, exempt) for item in node]
